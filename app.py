...
import requests
import edge_tts  # Requer pip install edge-tts
...
def despachar_geracao_audio(texto: str, motor: str, voz_edge: str = "") -> Optional[BytesIO]:
    """Seleciona qual motor de áudio usar"""
    if motor == "EdgeTTS (Neural)":
        return gerar_audio_edge(texto, voz_edge)
    else:
        # Fallback para gTTS
...
st.sidebar.markdown("### 🔊 Configuração de Áudio")
motor_audio_escolhido = st.sidebar.selectbox("Motor de Áudio", ["gTTS (Padrão)", "EdgeTTS (Neural)"], index=0)

voz_edge_escolhida = "pt-BR-FranciscaNeural"
if motor_audio_escolhido == "EdgeTTS (Neural)":
    # Vozes populares do EdgeTTS para português
    voz_edge_escolhida = st.sidebar.selectbox(
        "Voz (EdgeTTS)", 
        [
...
st.sidebar.info(f"Áudio: {motor_audio_escolhido}\nVídeo: {motor_escolhido}\nFormato: {resolucao_escolhida}")
...
with tab4:
    ...
    st.info(f"⚙️ Config: **Áudio: {motor_audio_escolhido}** | **Imagem: {motor_escolhido}**")
        ...
        if st.button("🔊 Gerar Todos os Áudios", use_container_width=True):
            with st.status("Gerando áudios em lote...", expanded=True) as status:
                ...
                for b in blocos_config:
                    ...
                    if txt:
                        st.write(f"Gerando áudio ({motor_audio_escolhido}): {b['label']}...")
                        try:
                            # Usa a nova função de despacho
                            audio = despachar_geracao_audio(txt, motor_audio_escolhido, voz_edge_escolhida)
                            if audio:
                                st.session_state["generated_audios_blocks"][bid] = audio
                                count += 1
                        except Exception as e:
                            st.error(f"Erro em {bid}: {e}")
                status.update(label=f"Concluído! {count}/{total} áudios gerados com {motor_audio_escolhido}.", state="complete")
                st.rerun()
        ...
        with st.container(border=True):
            ...
            with col_text:
                if bloco["text_key"]:
                    ...
                    # Botão individual com lógica de despacho
                    if st.button(f"🔊 Gerar Áudio ({block_id})", key=f"btn_audio_{block_id}"):
                        if txt_content:
                            try:
                                audio = despachar_geracao_audio(txt_content, motor_audio_escolhido, voz_edge_escolhida)
